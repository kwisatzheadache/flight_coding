defmodule Morphology do
  ...
  def set(scape, interactor) do
    ...
    case interactor do
      :sensor ->
        [%Interactor{id: {:sensor, Generate.id()}, name: get_input, scape: {:private, scape}, vl: 2, index: :sensor}]
      :actuator ->
        [%Interactor{id: {:actuator, Generate.id()}, name: send_output, scape: {:private, scape}, vl: 1, index: :actuator}]
    end
  end
